...
class SP_Testimonials_Widget extends WP_Widget {
    function __construct() {
        ...
        parent::__construct( 'sp_testimonials', __( 'WP Testimonials Slider', 'wp-testimonial-with-widget' ), $widget_ops, $control_ops );
    }
    function widget( $args, $instance ) {
		...
		if ( isset( $instance['slides_column'] ) && ( empty( $instance['slides_column'] ) ) ) { $args['slides_column'] = intval( $instance['slides_column'] ); }
    ...
    ?>  
    <div id="wptww-testimonial-<?php echo $unique; ?>" class="widget widget_sp_testimonials">
        <?php  if ( '' != $args['title'] ) {
                echo '<h2 class="widget-title">' . esc_html( $args['title'] ) . '</h2>' . "\n";
            }?>
        <div class="wptww-testimonials-slide-widget-<?php echo $unique; ?> wptww-testimonials-slide-widget <?php echo $design; ?>">
        <?php
          $count = 0;
            foreach ( $query as $post ) { 
			   ...
			   <?php if($args['slides_column'] != "" && $args['slides_column'] != "0"){
					$slidesToShow = $args['slides_column'];}else{$slidesToShow = 1;}
					if($args['slides_scroll'] != "" && $args['slides_scroll'] != "0"){
					$slidesToScroll = $args['slides_scroll'];} else{$slidesToScroll = 1;}?>
						jQuery(document).ready(function(){
						jQuery('.wptww-testimonials-slide-widget-<?php echo $unique; ?>').slick({
...
?>
        ...
        <p>
		 ...
		 <!-- Widget Order: height -->
        <p>
            <label for="<?php echo $this->get_field_id( 'adaptive_height' ); ?>"><?php _e( 'Adaptive Height:', 'wp-testimonial-with-widget' ); ?></label>
...
add_action( 'widgets_init',  function() {
             register_widget("SP_Testimonials_Widget");        
}, 1 );
